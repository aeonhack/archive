...
NotInheritable Class ServerClient
    ...
    'NOTE: This ServerClient has been modified so that the UserState property is of type User instead of Object 
    'so that excessive casting is not necessary.
...
#Region " Properties "
    ...
    Private _UserState As User
    Public Property UserState() As User
        Get
            Return _UserState
        End Get
        Set(value As User)
...
#Region " Properties "
    ...
    Private _SingleInstanceClients As Boolean
    Public Property SingleInstanceClients() As Boolean
        Get
            Return _SingleInstanceClients
        End Get
        Set(value As Boolean)
            If _Listening Then
                Throw New Exception("Unable to change this option while listening.")
            Else
                _SingleInstanceClients = value
            End If
        End Set
    End Property
...
#End Region
    ...
    Private Sub Process(s As Object, e As SocketAsyncEventArgs)
        Try
            If e.SocketError = SocketError.Success Then
                ...
                Dim CancelConnection As Boolean

                SyncLock _Clients
                    If _Clients.Count >= _MaxConnections Then
                        CancelConnection = True
                    ElseIf _SingleInstanceClients AndAlso AddressConnected(T.EndPoint.Address) Then
                        CancelConnection = True
                    End If
    ...
    Private Function AddressConnected(address As IPAddress) As Boolean
        For Each C As ServerClient In _Clients
            If C.EndPoint.Address.Equals(address) Then Return True
        Next
